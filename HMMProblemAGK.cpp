...
void HMMProblemAGK::destroy() {
	...
	// gradPIk,g
	if ( this->gradPI != NULL) {
		free2DNumber(this->gradPI, this->p->nK);
		this->gradPI = NULL;
	}
	// gradA
	if ( this->gradAk != NULL) {
...
NUMBER HMMProblemAGK::GradientDescentPLoSKillGroupOtherSkill() {
	...
	if(this->p->single_skill==1) {
		...
		while( !conv && iter<=this->p->maxiter ) {
            ...
            HMMProblemAGK::computeGradients();
			// add gradients
			for(k=0; k<this->p->nK; k++) {
                if(iter==1)
                    pO0 += HMMProblem::getSumLogPOPara(this->p->k_numg[k], this->p->k_g_data[k]);
				add1DNumbersWeighted(this->gradPI[k], gradPI, this->p->nS, 1.0);
			...
			for(k=0; k<this->p->nK; k++) {
                ...
                doLinearStep(this->p->k_numg[k], this->p->k_g_data[k], this->gradPI[k], this->gradAk[k], this->gradB[k], gradPI, gradA, gradB);
            }
        ...
        for(k=0; k<this->p->nK && skip_k<this->p->nK; k++) { // for all A,B-by-skill
            ...
            while( !conv && iter<=this->p->maxiter ) {
                ...
                if(iter==1) {
                    ...
                    //                        for(x=0; x<xndat; x++) {
                    //                            metrics[0] = 0.0;
                    //                            HMMProblemAGK::computeLogLikRMSE(metrics, false, 1, &x_data[x], this->PI[k], this->PIg, this->A[k], this->Ag, this->B[k], this->p);
                    ...
                    //                        }
                    //                        HMMProblemAGK::computeLogLikRMSE(metrics, false, this->p->k_numg[k], this->p->k_g_data[k], this->PI[k], this->PIg, this->A[k], this->Ag, this->B[k], this->p);
                    //                        free(metrics);
                    //                    }
                }
