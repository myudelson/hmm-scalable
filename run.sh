...
#                            do
                                ...
                                do
                                    ...
                                    then # no cross validation
                                        ...
                                        echo "./trainhmm -s "$s" -t 0.01 -q 0 -f "$f" -v "$v" -p "$p" -m "$m" -c "$c" -d ~ "$train_file" "$model_file" "$predict_file" > "$out_file
                                        ./trainhmm -s $s -t 0.01 -q 0 -f $f -v $v -p $p  -m $m -c $c -d ~ $train_file $model_file $predict_file > $out_file 2>&1
                                    fi  
#                                    gzip -9 $predict_file
                                    echo "..."
                                done # cros-validate
